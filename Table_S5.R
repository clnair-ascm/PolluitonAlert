...
# --- Calculate mean and SE for each pollutant by Level and City ---
summary_data <- combined_data %>%
  group_by(Level, City) %>%
  summarise(
    mean_pm25 = mean(abs_pm25_dif, na.rm = TRUE),
    se_pm25   = sd(abs_pm25_dif, na.rm = TRUE) / sqrt(sum(!is.na(abs_pm25_dif))),
    mean_pm10 = mean(abs_pm10_dif, na.rm = TRUE),
    se_pm10   = sd(abs_pm10_dif, na.rm = TRUE) / sqrt(sum(!is.na(abs_pm10_dif))),
    mean_so2  = mean(abs_so2_dif, na.rm = TRUE),
    se_so2    = sd(abs_so2_dif, na.rm = TRUE) / sqrt(sum(!is.na(abs_so2_dif))),
    mean_no2  = mean(abs_no2_dif, na.rm = TRUE),
    se_no2    = sd(abs_no2_dif, na.rm = TRUE) / sqrt(sum(!is.na(abs_no2_dif))),
    .groups = "drop"
  )

# --- Combine mean ± SE into formatted strings (1 decimal each) ---
summary_data <- summary_data %>%
  mutate(
    PM25 = sprintf("%.1f±%.1f", mean_pm25, se_pm25),
    PM10 = sprintf("%.1f±%.1f", mean_pm10, se_pm10),
    SO2  = sprintf("%.1f±%.1f", mean_so2, se_so2),
    NO2  = sprintf("%.1f±%.1f", mean_no2, se_no2)
  ) %>%
  select(City, Level, PM25, PM10, NO2, SO2)

# --- Pivot to wide format (City × [Pollutant_Level]) ---
tidy_summary <- summary_data %>%
  pivot_longer(cols = c(PM25, PM10, NO2, SO2),
               names_to = "Pollutant",
               values_to = "Value") %>%
  pivot_wider(names_from = c(Pollutant, Level),
              values_from = Value) %>%
  arrange(City) %>%
  select(City,
         starts_with("PM25_"),
