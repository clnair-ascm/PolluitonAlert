...
aqlist <- readRDS("/Volumes/daiyy/2024_paper/RedAlert/data/chap_era5_daily/aqmet_incalert_predv3.rds")
...
test_data <- combined_Orange %>%
  filter(
    City == "Yangquan",
    date >= ymd("2020-11-01") & date <= ymd("2021-02-28")
  )

plot(test_data$date, test_data$rlt_no2_dif)
plot(test_data$date, test_data$abs_no2_dif)
...
summary_data <- beijing_data %>%
  group_by(Level, City, Province) %>%
  summarise(
...
# for (i in seq_along(pollutants)) {
  ...
  # Save the plot
  #   ggsave(paste0("/Users/yqdai/Downloads/RedAlert/paper/", pollutant, ".tiff"), plot = plot, width = 12, height = 10, units = "in", dpi = 300)
# }
  plot
