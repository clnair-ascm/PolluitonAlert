...
df0 <- mort_by_city
df0$mort <- round(df0$mort_cf - df0$mort_bs)
df0$mort_upper <- round(df0$mort_cf_upper - df0$mort_bs_upper)
df0$mort_lower <- round(df0$mort_cf_lower - df0$mort_bs_lower)
...
df0$Province <- factor(df0$Province, levels = province_order)
...
test <- ggplot(df, aes(y = City)) +
  ...
  geom_segment(
    aes(x = mort_bs, xend = mort_cf, y = City, yend = City),
...
test
ggsave("/Volumes/shiz-wm-netzero/users/yuqing/PNAS_TR/data/pnas_tr/Shanxi.tiff", plot = test, width = 5, height = 4, units = "in", dpi = 300)
