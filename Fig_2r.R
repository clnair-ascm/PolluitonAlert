...
for (i in seq_along(pollutants)) {
  ...
  range_width <- diff(y_limit)
  # Choose ~6 ticks across the x-axis (adjust 'target_ticks' if you prefer more/less)
  target_ticks <- 6
  tick_interval <- pretty(c(y_limit[1], y_limit[2]), n = target_ticks)
  tick_breaks <- tick_interval
  # ------------------------------------------------
  ...
  plot <- ggplot(summary_data, aes(x = .data[[pollutant]], y = Province, fill = Level)) +
    geom_errorbarh(aes(xmin = .data[[pollutant]] - (.data[[error]]), 
                       xmax = .data[[pollutant]] + (.data[[error]])),
                   height = 0.2, color = "black", size = 1.2) +  # Bolder, wider error bars
    geom_point(color = "black", size = 9, shape = 21) +  # Larger mean dot
    geom_vline(xintercept = 0, linetype = "dashed", color = "black", size = 1.2) +  # Bolder vertical line
    scale_fill_manual(values = c("Yellow" = "#FFFF00", "Orange" = "#FFA500", "Red" = "#FF0000")) +
    theme_minimal(base_size = 18, base_family = "Arial") +
    # ---- Automatic tick scaling ----
    scale_x_continuous(
      breaks = tick_breaks,
      limits = y_limit,
      expand = c(0, 0)
    ...
    theme(
      ...
      axis.text = element_text(size = 32, color = "black"),
      axis.line.x.top = element_line(color = "black"),
      axis.text.y = element_text(margin = margin(r = 30)),   # ⬅️ add gap between labels & axis
      axis.line.y.right = element_line(color = "black"),
      panel.border = element_rect(color = "black", fill = NA, size = 1),
      panel.grid.major.x = element_line(color = "grey80", linetype = "dashed"),
      panel.grid = element_blank(),
  ...
  plot <- plot +
    theme(
      plot.margin = margin(t = 10, r = 20, b = 20, l = 10, unit = "pt")  # add more space below
    )
  ggsave(paste0("/Volumes/shiz-wm-netzero/users/yuqing/PNAS_TR/data/pnas_tr/", pollutant, ".tiff"), plot = plot, width = 12, height = 10, units = "in", dpi = 300)
 }
